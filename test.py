...
parser.add_argument('-c', '--channels', type=int, default=1,
                    help="Select the representation in terms of channels, 1 or 3 or 4")
parser.add_argument('-a', '--activation', type=str, default='relu', help='Select from relu/leakyrelu/prelu/elu')
parser.add_argument('-e', '--example_no', type=str, default='all',
                    help='Select the model from  the examples folder for evaluation eg. 1,2,3 etc. , else give all')
...
from loadModel import _loadModel
...
if __name__ == '__main__':
    ...
    cnnModel = _loadModel(model_no, channels, activation, voxelCount, nbClasses)
    cnnModel.load_weights('weights\model%s_%schannel_%sactivation_%svoxel_count_%sclasses.h5' % (
    model_no, channels, activation, voxelCount, nbClasses))
